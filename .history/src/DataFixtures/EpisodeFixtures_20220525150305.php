...
class EpisodeFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager): void
    {
      foreach (self::EPISODE as $key => $episodeName) {
        ...
        $episode = new Season();
        $episode->setNumber($seasonName['Number']);
        $episode->setYear($seasonName['Year']);
        $episode->setDescription($seasonName['Description']);
        $episode->setProgram($this->getReference('program_' . $seasonName['Program']));
