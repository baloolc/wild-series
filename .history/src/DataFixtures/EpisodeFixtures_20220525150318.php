...
class EpisodeFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager): void
    {
      foreach (self::EPISODE as $key => $episodeName) {
        ...
        $episode = new Season();
        $episode->setNumber($episodeN['Number']);
        $episode->setYear($episodeN['Year']);
        $episode->setDescription($episodeN['Description']);
        $episode->setProgram($this->getReference('program_' . $episodeN['Program']));
        $manager->persist($episodeN);
        }
        $manager->flush();
